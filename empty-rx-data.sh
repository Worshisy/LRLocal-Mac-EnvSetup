#!/usr/bin/env bash
# empty-rx-data.sh — DELETE all RX capture data on this Mac (mini), after
...
# [c] like field-jobs.sh find_dir, plus /Volumes/USRP* (repos live on the
...
if [ -n "$TMUX_BIN" ] && "$TMUX_BIN" has-session -t rx 2>/dev/null; then
  abort "the rx capture job is RUNNING — stop it first:  $HERE/field-jobs.sh stop rx"
fi
