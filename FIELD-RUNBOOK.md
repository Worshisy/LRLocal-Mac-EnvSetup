...
## 3. Long field jobs that survive SSH disconnect (`field-010-jobs.sh`)
...
~/LRLocal-Mac-EnvSetup/field-010-jobs.sh start # launches both in tmux, logs to ~/field-logs/
#   …now you can just close the SSH session; both keep running.

# later, reconnect and watch:
~/LRLocal-Mac-EnvSetup/field-010-jobs.sh attach rx   # live tmux view; Ctrl-b then d to detach
~/LRLocal-Mac-EnvSetup/field-010-jobs.sh logs   rtk  # or tail the log file
~/LRLocal-Mac-EnvSetup/field-010-jobs.sh status      # what's running
~/LRLocal-Mac-EnvSetup/field-010-jobs.sh stop        # stop both
```
...
- Starting **rx** always asks for the **RX center frequency** (Enter = keep the
  ...
  Pre-answer with `RX_FREQ=2.44e9 … field-010-jobs.sh start`.
...
## 4. SSH shortcuts + internet for an offline field mini (`host-010-ssh-config.sh`)

One-time **on the operator machine** (your laptop / lab Mac — not the mini):
```sh
./host-010-ssh-config.sh      # installs ssh shortcuts ddh-mac-01 … ddh-mac-06
```
...
```sh
~/LRLocal-Mac-EnvSetup/field-020-update-repos.sh
```
